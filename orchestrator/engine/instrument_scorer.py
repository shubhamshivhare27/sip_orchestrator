...
def _fixed_weight_split(etfs_with_weights: list[dict], total_budget: float) -> list[dict]:
    """
    For Thematic ETFs that have a target weight in allocation_config
    (ETF1=6%, ETF2=5%, ETF3=4%), split THIS MONTH'S thematic sleeve SIP
    budget (total_budget) proportionally across the active ETFs using
    their relative target weights (e.g. 6:5:4).  Mirrors the Hedge sleeve
    split below — NOT portfolio_value × weight, which would compute the
    ETF's full target portfolio value instead of a monthly deployable
    amount (was causing 50-60x budget overshoot in the execution plan).
    Returned with 'allocated_budget'.
    """
    if not etfs_with_weights or total_budget <= 0:
        for e in etfs_with_weights:
            e["allocated_budget"] = 0
        return etfs_with_weights

    total_weight = sum(e.get("target_weight", 0.05) for e in etfs_with_weights)
    if total_weight <= 0:
        per_etf = total_budget / len(etfs_with_weights)
        for e in etfs_with_weights:
            e["allocated_budget"] = round(per_etf, 2)
        return etfs_with_weights
    ...
    for e in etfs_with_weights:
        ...
        e["allocated_budget"] = round((weight / total_weight) * total_budget, 2)
    return etfs_with_weights
...
def score_instruments(
    ...
    portfolio_value     : Total portfolio ₹ value. Currently unused for budget math
                          (Thematic now splits thematic_budget directly, like Hedge) —
                          kept for future drift/diagnostic use by callers.
    ...
    if thematic_budget > 0 and sleeve_status.get("Thematic") != "STOP" and phase_etfs:
        ...
        thematic_candidates = _fixed_weight_split(thematic_candidates, thematic_budget)
