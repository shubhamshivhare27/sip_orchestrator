"""
orchestrator/main.py (v4 — instrument_scorer v5 patch)
─────────────────────────────────────────────────────────────────────────────
13-step pipeline: Sleeve-first allocation → Engine 2 priority → Tranche deployment → Rotation.
  ...
  Step 11  Score instruments + buy dates + tranche amounts   ← PATCHED (v5 scorer)
  Step 12  Compute exit actions
  Step 13  Write outputs + send email

PATCH NOTES (instrument_scorer v5):
  - score_instruments() now takes explicit sleeve_budgets, live_scores, phase_etfs,
    hedge_etfs, engine2_signals, sleeve_status, portfolio_value instead of the old
    (macro_data, signal_data, alloc_plan, config) signature.
  - Returns list[dict] instead of list[InstrumentScore].
  - Dicts are wrapped as SimpleNamespace so all downstream code (inst.ticker etc.)
    continues to work without changes.
  - Core/International/Hedge ETFs are now included based on LIVE 12-indicator scores.
    Engine 3 phase rotation drives ONLY the Thematic sleeve.
"""
...
def run(sip_amount=None, dry_run=False):
    ...
    if not dry_run and live_scores:
        nifty = (live_scores.get("NIFTYIETF") or live_scores.get("NIFTYBEES")
                 or live_scores.get("NIFTYIETF.NS") or live_scores.get("NIFTYBEES.NS"))
    ...
    # Engine 2 BUY signals as {ticker: "BUY"} dict
    _engine2_signals = {
        sig["ticker"].upper(): "BUY"
        for sig in signal_data.get("buy_signals", [])
    }

    # Sleeve status from alloc_plan: "BOOST", "STOP", "ON_TARGET" etc.
    _sleeve_status = {
        name: s.status
        for name, s in alloc_plan.sleeves.items()
    }

    # Sleeve budgets (same as already computed above, re-stated for clarity)
    _sleeve_budgets_v5 = {
        name: s.sip_allocation
        for name, s in alloc_plan.sleeves.items()
    }

    # Phase ETFs for Thematic sleeve — only ENTER/HOLD signals from Engine 3.
    # rotation_signals is a list of RotationSignal objects with .action, .ticker,
    # .new_weight (percentage, e.g. 6.0).
    _phase_etfs = [
        {
            "ticker":        sig.ticker,
            "rank":          i + 1,
            "target_weight": sig.new_weight / 100,   # convert % → fraction
        }
        for i, sig in enumerate(
            [s for s in rotation_signals if s.action in ("ENTER", "HOLD")]
        )
    ]
    ...
    if not _phase_etfs:
        ...
        if _phase_etfs:
            log.info(f"  phase_etfs fallback: using config for phase '{cycle_phase}': "
                     f"{[e['ticker'] for e in _phase_etfs]}")

    # Hedge ETFs — try config first, hard-coded fallback.
    _hedge_etfs = []
    hedge_cfg = config.get("sleeves", {}).get("Hedge", {})
    hedge_target_weights = hedge_cfg.get("target_weights", {})
    if hedge_target_weights:
        _hedge_etfs = [
            {"ticker": t, "target_weight": w}
            for t, w in hedge_target_weights.items()
        ]
    if not _hedge_etfs:
        _hedge_etfs = [
    ...
    # Call the v5 scorer — returns list[dict]
    scored_dicts = score_instruments(
        sleeve_budgets  = _sleeve_budgets_v5,
        live_scores     = _live_scores_dict,
        phase_etfs      = _phase_etfs,
        hedge_etfs      = _hedge_etfs,
        config          = config,
        engine2_signals = _engine2_signals,
        sleeve_status   = _sleeve_status,
        portfolio_value = weights.get("total_value", 0),
    )

    # ── Wrap dicts as SimpleNamespace so downstream code (inst.ticker etc.) ───
    # works without any further changes. Also set all fields that Steps 11–13
    # and the execution_plan JSON block expect.
    scored = []
    for item in scored_dicts:
        inst = SimpleNamespace(**item)

        # Budget field: v5 uses 'allocated_budget', downstream expects 'allocated_inr'
        inst.allocated_inr     = getattr(inst, "allocated_budget", 0)

        # Deployment fields (set by the loop below, initialise to safe defaults)
        inst.deploy_now_inr    = 0
        inst.deploy_tranche    = None
        inst.deploy_multiplier = 1.0
        inst.buy_date          = None
        inst.buy_date_rule     = ""
        inst.buy_date_source   = ""
        ...
        # Tag / stance (used in email and dashboard)
        inst.tag    = getattr(inst, "source", "")
        inst.stance = getattr(inst, "signal", "")

        # Price / RSI / momentum (pulled from live_scores if available)
        inst.price  = ls_obj.price  if ls_obj else None
        inst.rsi    = None
        inst.mom_4w = None
        if ls_obj and ls_obj.indicators:
            rsi_ind = next((i for i in ls_obj.indicators if i.name == "RSI Zone"), None)
            if rsi_ind:
                try: inst.rsi = float(rsi_ind.value)
                except: pass

        scored.append(inst)

    log.info(f"  instrument_scorer v5: {len(scored)} instruments in execution plan")

    # ── Step 11 continued: Buy dates + tranche amounts ────────────────────────
    ...
    for inst in scored:
        ...
        if matching_dep:
            ...
            if trigger == "ENGINE2" and not inst.has_engine2_signal:
                ...
                inst.buy_date_source   = "Engine 2 priority override"
                ...
                inst.buy_date_source   = "Engine 3 phase change"

        # Apply per-ETF structural rules (§7.2: MOMOMENTUM skip, MAFANG 2-tranche cap,
        # HSET 0.5×, GOLDBEES fixed 2nd Monday, per-ETF fallback dates, etc.)
        #
        # NOTE (v5 Priority-2 rebuild): this REPLACES the old _adjust_etf_multiplier
        # call. The old 4-arg function could only ever SCALE an already-nonzero
        # inst.deploy_now_inr — it had no access to `today` or `tranche_state`, so it
        # was structurally unable to force a fresh purchase when the sleeve itself
        # didn't trigger this run (e.g. GOLDBEES's fixed 2nd Monday, MID150BEES the
        # moment Golden Cross fires, or any per-ETF fallback date). The new function
        # is passed today/tranche_state and returns the FINAL deploy_now_inr directly.
        if live_scores:
            sleeve_letter = inst.deploy_tranche.replace("Tranche ", "").strip() if inst.deploy_tranche else None
            rule_result = tm.apply_etf_structural_rules(
                ticker=inst.ticker,
                allocated_inr=inst.allocated_inr,
                sleeve_proportional_inr=inst.deploy_now_inr,
                sleeve_multiplier=inst.deploy_multiplier,
                deploy_tranche_letter=sleeve_letter,
                config=config,
                live_scores=live_scores,
                tranche_state=tranche_state,
                today=today,
            )
            inst.deploy_now_inr    = rule_result["deploy_inr"]
            inst.deploy_multiplier = rule_result["multiplier"]
            if rule_result["tranche"]:
                inst.deploy_tranche = f"Tranche {rule_result['tranche']}"
            if rule_result["skip"]:
                inst.buy_date_rule = f"SKIP: {rule_result['note']}"
            elif rule_result["note"]:
                inst.buy_date_rule = rule_result["note"]

        ls = live_scores.get(inst.ticker)
        log.info(
            f"  {inst.ticker}: score={inst.composite} alloc=Rs.{inst.allocated_inr:,.0f} "
    ...
    # Persist per-ETF structural-rule state once (purchase counts, carry-forward
    # cash for MOMOMENTUM, etc.) — apply_etf_structural_rules mutates tranche_state
    # in-memory per call but doesn't write to disk per-instrument to avoid redundant I/O.
    tm.save_state(tranche_state, INPUTS_DIR)
    ...
    result = {
        "meta": {
            ...
            "dry_run":            dry_run,
            "pipeline_version":   "v4",
            "scorer_version":     "v5",          # track the upgrade
        },
        "sleeve_status": {
            name: {
                "label":           s.label,
