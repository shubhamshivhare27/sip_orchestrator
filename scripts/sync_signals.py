...
if snapshot:
    ...
    # Fallback: use target percentages
...
try:
    ...
    log.info(f"  RSI={rsi_val:.1f if rsi_val else 'N/A'} VIX={vix_val:.1f if vix_val else 'N/A'} "
             f"SMA50={sma_val:.1f if sma_val else 'N/A'}% Weekly={wk_ret:.1f if wk_ret else 'N/A'}%")
    ...
    if signal_data:
        ...
        for s, sc in config["sleeves"].items():
            for t in sc.get("instruments",[]): ticker_to_sleeve[t.upper()] = s
