    ...
    if not insts: st.info("No eligible instruments this run.")
        ...
        for i in insts:
            rows.append({"ETF":i["ticker"].replace(".NS",""),"Sleeve":i["sleeve"],"Tag":i["tag"],
                "Amount":i["allocated_inr"],"Buy Date":i["buy_date"],"Score":i["composite"],
        ...
        st.dataframe(pd.DataFrame(rows),use_container_width=True,hide_index=True,
            column_config={"Amount":st.column_config.NumberColumn(format="₹%d"),"Score":st.column_config.ProgressColumn(min_value=0,max_value=100)})
        for i in insts:
...
with tab8:
    ...
    if summary:
        ...
        s2.metric("Deployed",inr(summary.get("total_deployed",0)))
        s3.metric("Base SIP",inr(summary.get("total_base",0)))
        s4.metric("Remaining",str(summary.get("remaining_tranches",3)))
